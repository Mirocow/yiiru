 ...
 *
 * Класс адаптирован из библиотеки ADOdb Date, как части
 * {@link http://phplens.com/phpeverywhere/ библиотеки абстракций ADOdb}.
 ...
 *     All rights reserved.
 *
 * Встроенные функции PHP по работе с датами используют для расчетов целочисленные временные отметки.
 * По этой причине даты ограничены рамками 1901-2038 года в Unix и
 * 1970-2038 года в Windows из-за переполнения границ целых чисел для дат за этими рамками годов.
 * Данная библиотека преодоляет данные ограничения заменой
 * встроенных знаковых целых чисел (обычно - 32 бита) числами с плавающей точкой
 * (обычно - 64 бита).
 *
 * Были протестированы даты между 100 годом н.э. и 3000 годом н.э. и больше. Минимальный год -
 * 100 н.э., т.к. для годов меньше 100 будет выполнена конвертация 2 => 4 (YY => YYYY).
 * Максимально - миллиарды лет в будущем, но это теоретический предел,
 * т.к. вычисление такого года займет слишком много времени в текущей реализации
 * метода {@link getTimestamp}.
 *
 * ПРОИЗВОДИТЕЛЬНОСТЬ
 * Для высокой скорости библиотека использует встроенные функции дат там, где это возможно,
