...
class CDbCriteria extends CComponent
{
	...
	public $with;
	/**
	 * @var string псевдоним таблицы. Если не установлен, используется строка 't'
	 */
	public $alias;
	/**
	 * @var boolean должны ли внешние таблицы быть связаны с первичой таблицей в одном SQL-запросе.
	 * Свойство используется только в Active Record-запросах.
	 *
	 * Если данное свойство установлено в значение true, то для реляционного AR-запроса будет выполнен лишь один SQL-запрос,
	 * даже если первичная таблица лимитирована (установлено ограничение количества записей) и
	 * связь между внешней и первичной таблицами является связью многие-к-одному.
	 *
	 * Если данное свойство установлено в значение false, для каждой связи HAS_MANY будет выполнен отдельный SQL-запрос.
	 *
	 * Если данное свойство не установлено, если первичная таблица лимитирована, то для каждой связи HAS_MANY
	 * будет выполнен отдельный SQL-запрос, иначе будет выполнен единственный SQL-запрос
	 *
	 * @since 1.1.4
	 */
	public $together;
	/**
	 * @var string имя AR-атрибута, значение которого должно использоваться в качестве индекса массива результата запроса.
	 * По умолчанию - null, т.е., массив результата будет иметь целочисленные индексы, начинающиеся с нуля
	 * @since 1.1.5
	 */
	public $index;
	/**
     ...
     * <li>Несколько групп условий: $criteria->scopes=array('scopeName1','scopeName1');</li>
     * <li>Группа условий с параметрами: $criteria->scopes=array('scopeName'=>array($paramters));</li>
     * <li>Несколько групп условий с одним именем и разными параметрами: array(array('scopeName'=>array($paramters1)),array('scopeName'=>array($paramters2)));</li>
     * </ul>
     * @since 1.1.7
     */
	public $scopes;
